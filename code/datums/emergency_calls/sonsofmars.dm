...
/datum/emergency_call/som/print_backstory(mob/living/carbon/human/H)
	...
	to_chat(H, "")
	to_chat(H, "<B>Today, a USCM vessel, [SSmapping.configs[SHIP_MAP].map_name], has sent out a distress signal on the orbit of [SSmapping.configs[GROUND_MAP].map_name]. This is our chance to attack without being intercepted!</b>")
	to_chat(H, "<B>Eliminate the USCM, take no prisoners. Get back what it was once lost.</B>")
...
/datum/emergency_call/som/create_member(datum/mind/M)
	...
	if(!leader)
		...
		to_chat(H, "<span class='notice'>You are a member of the Sons of Mars assigned to lead this fireteam to the USCM distress signal sent out nearby. Lead your fireteam to top-working conidition!</span>")
		return

	if(medics < max_medics)
		var/datum/job/J = SSjob.GetJobType(/datum/job/som/medic)
		H.apply_assigned_role_to_spawn(J)
		to_chat(H, "<span class='notice'>You are a Sons of Mars medic assigned to this fireteam to respond to the USCM distress signal sent out nearby. Keep your squad alive in this fight!</span>")
		medics++
		return
	...
	if(prob(20))
		...
		H.apply_assigned_role_to_spawn(J)
		to_chat(H, "<span class='notice'>You are a veteran of the Sons of Mars and are assigned to this fireteam to respond to the USCM distress signal sent out nearby. Do them proud and kill all who stand in your teams way!</span>")
		return

	var/datum/job/J = SSjob.GetJobType(/datum/job/som/standard)
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, "<span class='notice'>You are a member of the Sons of Mars assigned to compose this fireteam to the USCM distress signal sent out nearby. Protect yourself and your other teammembers, kill all who stand in your team's way!</span>")
