...
/datum/emergency_call/pmc/create_member(datum/mind/M)
	...
	if(!leader)
		...
		to_chat(H, "<span class='notice'>You are the leader of this Weyland-Yutani contractor team in responding to the TGMC distress signal sent out nearby. Address the situation and get your team to safety!</span>")
		return
	...
	if(prob(30))
		...
		H.apply_assigned_role_to_spawn(J)
		to_chat(H, "<span class='notice'>You are a Weyland-Yutani heavy sniper assigned to this team to respond to the TGMC distress signal sent out nearby. Support your squad with long ranged firepower!</span>")
		return

	var/datum/job/J = SSjob.GetJobType(/datum/job/pmc/standard)
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, "<span class='notice'>You are a Weyland-Yutani contractor assigned to this team to respond to the TGMC distress signal sent out nearby. Assist your team and protect NT's interests whenever possible!</span>")
